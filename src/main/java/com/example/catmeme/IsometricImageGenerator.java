 ...
 */
public class IsometricImageGenerator extends Application {
    ...
    private void generateFloorImages() throws IOException {
        ...
        for (int i = 0; i < 50; i++) {
            ...
            saveImage(image, "src/main/resources/sol/floor_" + i + ".jpg");
        }
    }
    ...
    private void generateWallImages() throws IOException {
        ...
        for (int i = 0; i < 50; i++) {
            ...
            saveImage(image, "src/main/resources/murs/wall_" + i + ".jpg");
        }
    }
    ...
    private void generateCeilingImages() throws IOException {
        ...
        for (int i = 0; i < 30; i++) {
            ...
            saveImage(image, "src/main/resources/plafonds/ceiling_" + i + ".jpg");
        }
    }
    ...
    private void saveImage(WritableImage image, String path) throws IOException {
        // Conversion manuelle de WritableImage vers BufferedImage
        ...
        // Copier pixel par pixel - fond blanc pour JPG
        for (int x = 0; x < width; x++) {
            for (int y = 0; y < height; y++) {
                Color color = pixelReader.getColor(x, y);

                // Pour JPG, mélanger avec fond blanc selon l'alpha
                double alpha = color.getOpacity();
                int red = (int) ((color.getRed() * alpha + (1 - alpha)) * 255);
                int green = (int) ((color.getGreen() * alpha + (1 - alpha)) * 255);
                int blue = (int) ((color.getBlue() * alpha + (1 - alpha)) * 255);
        ...
        if ("JPG".equals(extension) || "JPEG".equals(extension)) {
            ...
            // Pour PNG, créer une image avec transparence
